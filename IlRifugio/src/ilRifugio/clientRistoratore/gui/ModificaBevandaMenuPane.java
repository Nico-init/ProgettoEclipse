...
public class ModificaBevandaMenuPane extends BorderPane {
	...
	private IBevanda bevandaDaVisualizzare;
	
	public ModificaBevandaMenuPane(IControllerOrdine controllerO, IControllerAccount controllerA, IControllerMenu controllerM, IControllerLog controllerL) {
		this.controllerO = controllerO;
		this.controllerA = controllerA;
		this.controllerM = controllerM;
		this.controllerL = controllerL;
		bevandaDaVisualizzare = ClientRistoratoreApp.getBevandaDaVisualizzare();
		
		onlyPane = new VBox();
		onlyPane.setAlignment(Pos.CENTER);
		onlyPane.setSpacing(10);
		
		Label ordineL = new Label("Modifica Bevanda");
		...
		tfNome.setText(bevandaDaVisualizzare.getNome());
	...
	private void handler(Button inserisci) {
		...
		double dPrezzo = 0;
		String nome = bevandaDaVisualizzare.getNome();
		...
		try {
			controllerM.modificaBevanda(nome, dPrezzo);
